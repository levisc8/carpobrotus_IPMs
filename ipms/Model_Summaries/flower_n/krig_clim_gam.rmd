...
all seasonal interactions models make top 3, and aren't separable from each other. 

## seasonal mods

temp_dry shows negative effect of big plants in drier areas, with 0-ish effect elsewhere. temp_wet is reverse - 0 effect for everything but big plants, which show a gain.

precip_dry shows a curvy surface for prec_dry, w/ peak around +1 SD. Prec_wet shows peak in top left (~-1SD) and negative elsewhere. 

SWs have an L shape for both seasons, w peaks in top left and bottom right. I suspect the bottom right is driven largely by Struisbaai, where everyhing has just 1 flower and and one or two are teeny-tiny. 

Precip seems to have largest magnitude.

## Annual model

sw2 is the only one that's within WAIC SE. 

Mean temperature shows more benefit across the board for warm temps, with more benefit to larger plants than smaller plants. Seasonality shows opposite pattern, with bigger plants doing well in less seasonal environments. 

Precip shows hump for big plants centered at ~+1SD, and then a peak for smallest plants around 2 SDs (likely driven by clone fragments in NZ). Seasonality shows biggest bump for tiny plants at +1 SD, and ~0 effect for everything else, except negative for big plants +1SD.

Mean SW shows 0 effect mostly, w/ big plants in very wet places doing worse. There is a big bump for tiny plants in +2SD SW seasonality. 

## Overall

Consistent effects in seasonal models, w/ precip always the largest in magnitude. Annual model suggest net effect is bigger plants do better in seasonal envs, but seasonal models give better detail, so think I'd stick w/ those for now. 
...
gam_list <- readRDS('../../Model_Fits/ramet_flower_n_list_krig_gam.rds') %>%
  .[[1]] %>%
  .[!grepl("waic", names(.))] %>%
  setNames(paste0("gam_", names(.)))
lin_list <- readRDS('../../Model_Fits/ramet_flower_n_list_krig.rds') %>%
  .[[1]] %>%
  .[!grepl("waic", names(.))]
