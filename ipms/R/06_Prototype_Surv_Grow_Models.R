...
surv_int_only <- glmer(alive ~ 1 + (1 | population),
                       data = surv_data,
                       family = binomial())

surv_ran_int <- glmer(alive ~ log_size + (1 | population),
                       data = surv_data,
                       family = binomial())

surv_int_slope_ran <- glmer(
              alive ~ log_size + (log_size | population),
  data    = surv_data,
  family  = binomial(),
...
grow_int_only <- lmer(log_size_next ~ 1 + (1 | population),
                       data = grow_data,
                      REML = FALSE)

grow_ran_int <- lmer(log_size_next ~ log_size + (1 | population),
                      data = grow_data,
                     REML = FALSE)

grow_ran_slope_int <- lmer(log_size_next ~ log_size + (log_size | population),
                           data = grow_data,
                           REML = FALSE)

# Exponential variance decay

grow_int_only_var <- lme(log_size_next ~ 1,
                         random = ~ 1 | population,
                         data = grow_data,
                         weights = varExp(form = ~ log_size | population), 
                         na.action = "na.omit",
                         control = lmeControl(maxIter = 500, 
                                              msMaxIter = 200),
                         method = "ML")

grow_ran_int_var <- lme(log_size_next ~ log_size,
                        random = ~ 1 | population,
                        data = grow_data,
                        weights = varExp(form = ~ log_size | population), 
                        na.action = "na.omit",
...
grow_ran_slope_int_var <- lme(log_size_next ~ log_size,
                              random = ~ log_size | population,
                              data = grow_data,
                              weights = varExp(form = ~ log_size | population), 
                              na.action = "na.omit",
                              control = lmeControl(maxIter = 500, 
...
anova(grow_int_only,
      grow_ran_int,
      grow_ran_slope_int)
anova(grow_int_only_var,
      grow_ran_int_var,
      grow_ran_slope_int_var)
