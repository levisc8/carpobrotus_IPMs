...
cat("**", clim, func, "**", sep = " ** ")
cat("\n\nDemography input file: ", demo_fp, "\n\nClim input file: ", occ_fp, "\n")
...
cat("Center var index: ", center_ind)
...
lapply(center_ind,
       function(x, data) {
         print(c(mean = mean(data[ , x], na.rm = TRUE), 
...
repro_mod <- brm(repr_bf,
                 ...
                 save_model = paste(out_dir,
                                    '/stan_code/repro_code_test.stan',
                                    sep = ""),
...
flower_n_mod <- brm(f_n_bf,
                    ...
                    cores = n_cores,
                    save_model = paste(out_dir,
                                       '/stan_code/f_n_code_test.stan',
                                       sep = ""),
                    control = list(adapt_delta = 0.99,
                                   max_treedepth = 15))

cat("Total time: ", Sys.time() - start)